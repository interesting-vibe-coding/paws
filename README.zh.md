...
| 按键 | 功能 |
|------|------|
| **CMD+G** | 首次按下创建游戏标签页，之后在 Agent ↔ 游戏之间切换 |
| **CMD+SHIFT+G** | 开关自动导航模式 |

### 模式

- **手动模式**（默认）— 自己按 CMD+G 跳到游戏。Agent 完成时（`stop`），Paws 自动把你切回 Agent 标签页。
- **自动模式** — Agent 开始工作时（`userPromptSubmit`）自动跳到游戏，完成时（`stop`）自动切回。完全无需动手。
...
### 已完成
- [x] 原生 CMD+G 创建 + 切换（纯 Lua，`wezterm.mux`，基于标签页）
- [x] Agent 完成时自动切回（OSC user var + `user-var-changed`）
- [x] 自动导航模式（CMD+SHIFT+G 切换；`userPromptSubmit` → 游戏，`stop` → Agent）
- [x] 通过 Agent skill 一键安装
- [x] `paws` Rust 启动器，每日轮换游戏（2048 / 数独 / 俄罗斯方块）

### 接下来（按优先级）
1. **暂停覆盖层** — Agent 完成时暂停游戏，显示覆盖层 + 自动返回倒计时（Rust 包装器的下一层）。
2. **更多更好的游戏** — 扩充精选列表；更丰富的游戏到位后替换掉 2048。
3. **Claude Code 支持** — notification / stop hooks。
